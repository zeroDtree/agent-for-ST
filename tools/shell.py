...
def run_shell_command_popen_tool(command: str) -> str:
    ...
    try:
        ...
        if CONFIG.get("restricted_mode", False):
            try:
                from utils.path_validator import get_safe_working_directory, validate_command_paths
                ...
                if detected_paths:
                    log_command_execution(
                        ...
                        "system",
                        "path_check",
                        f"Validated paths: {', '.join(detected_paths)}",
                    )
            ...
            if working_dir and os.path.exists(working_dir):
                # If working directory is configured and exists, use the
